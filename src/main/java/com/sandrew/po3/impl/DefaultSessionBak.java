 ...
 * @version    :
 */
public class DefaultSessionBak implements Session
{
	...
	private static Logger logger = LogManager.getLogger(DefaultSessionBak.class);
	 ...
	 * @param timeout
	 */
	protected DefaultSessionBak(String dbName, String txnName, NativeJdbcExtractor extractor, int timeout)
	{
		this.dbName = dbName;
	 ...
	 * @return
	 */
	public static DefaultSessionBak getInstance(String dbName, String txnName, NativeJdbcExtractor extractor, int timeout)
	{
		return new DefaultSessionBak(dbName, txnName, extractor, timeout);
	}
	...
	public Object callFunction(String functionName, List<Object> ins, int outType)
	{
		...
		try
		{
			...
			// 获取Connection
			Connection conn = DB2Manager.getConnection(dbName, txnName, timeout);
...
//		}
	...
	public byte[] readBlob(String colName, String sql, List<Object> params) throws POException
	{
		...
		try
		{
			conn = DB2Manager.getConnection(dbName, txnName, timeout);
			ps = conn.prepareStatement(sql);
			for (int i = 0; i < params.size(); i++)
			{
